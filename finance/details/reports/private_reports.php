...
<? 
        ...
        foreach ($finance_total as $finance_total) { 
            ?>
    <a onchange="this.form.submit()" href="private_reports.php?month=<?=$monthget?>&year=<?=$finance_total[0]?>"><li><?=$finance_total[0];?></li></a>
               <?
           }
        ?>
        </ul>
...
if (($monthget==$key) || (date('m')==$key && $i==0 && $monthget<date('m'))){ 
    $i=1;
    ?>
    <a class="current_month_reports" onchange="this.form.submit()" href="private_reports.php?month=<?=$key?>&year=<?=$select_year?>"><li><?=$month_lists?></li></a> <?
    }else{ 
    ?><a onchange="this.form.submit()" href="private_reports.php?month=<?=$key?>&year=<?=$select_year?>"><li><?=$month_lists?></li></a><?
}
            }?>
        </ul>
    </div>
<table class="table table-hover">
                        ...
                        foreach ($filter as $filters) {
                            ...
                            if (($month == $select_month && $year == $select_year) && ($filters[2]==$category1)) {
                        ?>
                                <tr>
