...
if (($monthget==$key) || (date('m')==$key && $i==0 && $monthget<date('m'))){ 
    $i=1;
    ?>
    <a class="current_month_reports" onchange="this.form.submit()" href="Food_reports.php?month=<?=$key?>"><li><?=$month_lists?></li></a> <?
    }else{ 
    ?><a onchange="this.form.submit()" href="Food_reports.php?month=<?=$key?>"><li><?=$month_lists?></li></a><?
}
            }?>
        </ul>
    </div>
...
if($filters[5]=='Общее') {$total_Common=$total_Common+$filters[4]; }    
                        ...
                        if (($month == $monthget && $year == date('Y')) && ($filters[2]==$category)) {
